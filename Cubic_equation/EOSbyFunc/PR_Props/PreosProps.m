function [Z, H, S, U, phi, info] = PreosProps(Tref, Pref, T, P, match,num)
...
%******************** Input section, can be edited **************
    ...
    propsRow = num; %enter the row number from the table, not the ID number
...
if(~exist('props','var')) % check if props database is loaded.
    db = load('props.mat'); %load as structured variable.
...
global R
   obj1 = (Z(index)*R*T/P)/88.4208 - 1;
   obj2 = U(index)/(-5333.5) - 1;
   obj = obj1.^2 + obj2.^2;
end
