function flash_Tbub(P,init_guess)
%{
    Calculates bubble Temperature(celsius) at given pressure(bar)
...
%{
    ...
    sum(yi) - 1 == 0
%}
func = @(x) z(1)*Psat1(x)/P + z(2)*Psat2(x)/P - 1; 
[T]  = fzero(func,init_guess,opt);
y1 = (z(1)*Psat1(T)/P);
y2 = (z(2)*Psat2(T)/P);

fprintf('Bubble Temperature = %.2f \n',T)
fprintf('y1 = %.3f | y2 = %.3f \n',y1,y2)
